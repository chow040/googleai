        ...
        >
           ...
           <div className="flex items-center gap-4 min-w-[180px]">
               <div>
                  ...
                  <div className="text-xs text-slate-400 truncate max-w-[140px]">{displayData.name}</div>
               </div>
           ...
           <div className="hidden sm:flex items-center gap-6 flex-1 justify-center">
                <div className="flex flex-col items-end min-w-[80px]">
                    ...
                    <div className={`text-xs font-bold flex items-center gap-1 ${isPriceUp ? 'text-emerald-400' : 'text-red-400'}`}>
                        {isPriceUp ? <TrendingUp className="w-3 h-3"/> : <TrendingDown className="w-3 h-3"/>}
                        {displayData.change}
                    </div>
                </div>
                
                <div className={`px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider border ${verdictColor} min-w-[60px] text-center`}>
                    {displayData.verdict}
                </div>
           </div>
           ...
           <div className="flex items-center gap-3">
                {displayData.rocketScore !== undefined && (
                    <div className="flex items-center gap-1.5 bg-slate-900/50 px-2.5 py-1.5 rounded-lg border border-white/5" title="Rocket Score">
    ...
    >
       ...
       <div className="flex justify-between items-start mb-4">
          <div className="min-w-0 pr-4">
             ...
             <div className="text-xs text-slate-400 truncate mt-0.5">{displayData.name}</div>
          </div>
          {!displayData.isNew && (
            <button 
                onClick={(e) => { e.stopPropagation(); onAction(e); }}
       ...
       <div className="space-y-4">
           ...
           <div className="flex justify-between items-baseline">
                <div className="text-2xl font-mono font-bold text-white tracking-tight">{displayData.price}</div>
                <div className={`text-sm font-bold flex items-center gap-1 ${isPriceUp ? 'text-emerald-400' : 'text-red-400'}`}>
                    {isPriceUp ? <TrendingUp className="w-3 h-3"/> : <TrendingDown className="w-3 h-3"/>}
                    {displayData.change}
                </div>
           </div>
           ...
           <div className="flex items-center justify-between gap-2">
                ...
                {displayData.rocketScore !== undefined && (
                    <div className="flex items-center gap-1.5 bg-slate-900/50 px-2 py-1 rounded-lg border border-white/5" title="Rocket Score">
...
const Dashboard: React.FC<DashboardProps> = ({
  ...
  const filteredLibrary = useMemo(() => {
      ...
      switch (filter) {
        ...
        case 'HIGH_SCORE':
          return items.filter(i => (i.fullReport?.rocketScore || 0) >= 80);
        default:
          return items;
      }
  ...
  const validScores = reportLibrary
      .map(r => r.fullReport?.rocketScore)
  ...
  const StatFilterCard = ({ 
    ...
    <div 
      onClick={onClick}
      className={`
        relative overflow-hidden rounded-xl p-4 border h-24 flex flex-col justify-between cursor-pointer transition-all duration-300 group
        ${active 
          ? `bg-slate-800 border-indigo-500 ring-1 ring-indigo-500/50 shadow-lg shadow-indigo-500/10` 
          : 'bg-surface border-white/5 hover:bg-slate-800 hover:border-indigo-500/30'
        }
      `}
    >
        <div className={`absolute top-0 right-0 p-3 transition-opacity ${active ? 'opacity-20' : 'opacity-10 group-hover:opacity-20'}`}>
          <div className={`transform -rotate-12 ${colorClass}`}>
             {icon}
          </div>
        </div>
        
        <div className={`text-[10px] font-bold uppercase tracking-widest z-10 transition-colors ${active ? 'text-indigo-300' : 'text-slate-500 group-hover:text-indigo-400'}`}>
           {label}
        </div>
        
        <div className={`text-3xl font-mono font-bold z-10 ${colorClass}`}>
           {value}
        </div>
        ...
        <div className="w-full max-w-4xl relative z-20">
          <form onSubmit={(e) => onSearch(e)} className="relative group">
              ...
              <div className="relative" ref={dropdownRef}>
                <div className="relative flex items-center bg-slate-900 rounded-xl shadow-2xl ring-1 ring-white/10 overflow-hidden">
                  ...
                  <div className="pr-2">
                    <button
                        ...
                        className="bg-white text-slate-900 px-6 py-3 font-bold hover:bg-indigo-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed rounded-lg flex items-center gap-2 text-sm tracking-wide"
                    >
