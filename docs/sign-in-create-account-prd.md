...
4. Track authentication state via Supabase session storage so the dashboard, settings, and save/bookmark flows react instantly without refresh.
...
- `localStorage` keys in play: `ultramagnus_user`, `ultramagnus_library_v1`, `ultramagnus_guest_usage`, and optional `ultramagnus_user_api_key`.
- When we connect to Supabase, `VITE_SUPABASE_URL` + anon key from `.env.local` will drive real auth. Plan to replace the fake timeout within `AuthModal` with a Supabase RPC and persist JWT/session tokens securely (never in plain localStorage without refresh tokens).
